...
const TIPOS = [
  'Entrada', 'Saida',
  'EdicaoDemanda - Criacao', 'EdicaoDemanda - Alteracao', 'EdicaoDemanda - Excluir',
  'EdicaoDemanda - Alteracao Massa', 'EdicaoDemanda - Cadastro em Massa', 'EdicaoDemanda - Upload Ficha',
  'ListaDemandas - Exportacao Excel', 'ListaDemandas - Exportacao PDF',
  'Admin - Criacao Usuario', 'Admin - Alteracao Usuario', 'Admin - Alteracao Permissoes',
];
...
export default function AdminLogs() {
  ...
  const colunas: Coluna<LogTransacao & { usuario_nome: string }>[] = [
    ...
    { key: 'cd_item_transacao', titulo: 'CD' },
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Auditoria completa — login, alterações, exclusões e exportações
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            onClick={() => exportarExcel(colunas.map((c) => ({ key: c.key, titulo: c.titulo })), data ?? [], 'OPP_Logs')}
          >
            <FileDown /> Exportar
          </Button>
        </div>
