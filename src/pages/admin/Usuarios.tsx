...
export default function AdminUsuarios() {
  ...
  const criar = useMutation({
    mutationFn: async () => {
      ...
      // O trigger do banco sempre cria como 'usuario'; promoção a admin
      // é feita em seguida pelo update (que exige fn_is_admin()).
      ...
      if (nova.user) {
        await supabase
          .from('usuarios')
          .update({ nome: form.nome, perfil: form.perfil })
          .eq('id', nova.user.id);
      }
  ...
  const salvarEdicao = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        .from('usuarios')
        .update({ nome: edicao.nome, perfil: edicao.perfil })
        .eq('id', edicao.id);
      if (error) throw error;
